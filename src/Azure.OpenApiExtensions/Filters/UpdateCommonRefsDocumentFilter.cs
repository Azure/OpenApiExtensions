// <copyright file="UpdateCommonRefsDocumentFilter.cs" company="Microsoft">
...
namespace Microsoft.Azure.Sentinel.ServiceHostingTools.SwaggerExtensions.Filters
{
    ...
    {
        public const string CommonPrefix = "../../../../../common-types/resource-management/v3/types.json";
        public const string RPCommonPrefix = "../../../common/1.0/types.json";
        public const string VersionCommonPrefix = "./common/";
        ...
        public UpdateCommonRefsDocumentFilter(SwaggerConfig swaggerConfig)
        {
            ...
            //this.commonDefinitions = swaggerConfig.CommonDefinitionModels ?? Array.Empty<string>().ToList();
            //this.commonParameters = swaggerConfig.CommonParameterModels ?? Array.Empty<string>().ToList();
            //this.rpCommonParameters = swaggerConfig.ResourceProviderParameterModels ?? Array.Empty<string>().ToList();
        }
        ...
        public void Apply(OpenApiDocument swaggerDoc, DocumentFilterContext context)
        {
            ...
            {
                var commonizePropertiesFixingActions = CommonizeSchemaPoperties(schema);
                ...
                if (_config.ResourceProviderReusableParameters.Contains(schema.Key))
                {
                    ...
                    fixingActions.Add(() =>
                    {
                        ...
                        {
                            Reference = new OpenApiReference { ExternalResource = $"{RPCommonPrefix}{ParametersPrefix}{schema.Key}" },
                ...
                if (_config.VersionCommonReusableDefinitions.ContainsKey(schema.Key))
                {
                    ...
                    fixingActions.Add(() =>
                    {
                        var refSchema = new OpenApiSchema
                        {
                            Reference = new OpenApiReference { ExternalResource = $"{VersionCommonPrefix}{_config.VersionCommonReusableDefinitions[schema.Key]}.json{DefinitionsPrefix}{schema.Key}" },
                            Description = schema.Value.Description
                        };
                        swaggerDoc.Components.Schemas.Remove(schema.Key);
                        swaggerDoc.Components.Schemas.Add(schema.Key, refSchema);
        ...
        private List<Action> CommonizeSchemaPoperties(KeyValuePair<string, OpenApiSchema> schema)
        {
            ...
            foreach (var prop in schema.Value.Properties)
            {
                ...
                var id = prop.Value.AllOf?.FirstOrDefault()?.Reference?.Id;
                if (id != null &&
                    ...
                    _config.VersionCommonReusableDefinitions.ContainsKey(id)))
                {
                    ...
                    fixingActions.Add(() =>
                    {
                        var referenceLink = _config.VersionCommonReusableDefinitions.ContainsKey(id) ?
                            $"{VersionCommonPrefix}{_config.VersionCommonReusableDefinitions[id]}.json{DefinitionsPrefix}{id}" :
        ...
        private void CommonizeOperationParameter(KeyValuePair<OperationType, OpenApiOperation> op)
        {
            ...
            foreach (var resuseParam in _config.GlobalCommonReusableParameters)
            {
                ...
                if (opParam != null)
                {
                    opParam.Reference = new OpenApiReference { ExternalResource = $"{CommonPrefix}{ParametersPrefix}{reuseParamName}" };
                }
            }
            ...
            foreach (var resuseParam in _config.ResourceProviderReusableParameters)
            {
                ...
                if (opParam != null)
                {
                    opParam.Reference = new OpenApiReference { ExternalResource = $"{RPCommonPrefix}{ParametersPrefix}{reuseParamName}" };
                }
            }
            ...
            foreach (var resuseParam in _config.VersionCommonReusableDefinitions)
            {
                ...
                if (opParam != null)
                {
                    opParam.Reference = new OpenApiReference { ExternalResource = $"{VersionCommonPrefix}{reuseParamFileName}.json{ParametersPrefix}{reuseParamName}" };
                }
            }
        }
